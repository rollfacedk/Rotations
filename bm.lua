...
local function APL ()
    ...
    if S.LightsJudgment:IsReady() and RubimRH.CDsON() and Pet:BuffP(S.FrenzyBuff) and Pet:BuffRemainsP() > Player:GCD() or not Pet:BuffP(S.FrenzyBuff) then
    ...
    if S.BarbedShot:IsReady() and Pet:BuffDownP(S.FrenzyBuff) and (S.BarbedShot:ChargesFractionalP() > 1.8 or Player:BuffP(S.BestialWrathBuff)) or S.AspectoftheWild:CooldownRemainsP() < Pet:BuffRemainsP(S.FrenzyBuff) - Player:GCD() and PrimalInstincts:AzeriteEnabled() then
