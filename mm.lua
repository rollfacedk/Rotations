...
local function APL ()
    ...
    St = function()
        ...
        if S.AimedShot:IsReadyP() and (Player:BuffDownP(S.PreciseShotsBuff) or Player:Focus() < 80 or Player:BuffP(S.TrueshotBuff)) then
            return S.AimedShot:Cast()
        end
        ...
        if S.ArcaneShot:IsCastableP() and (Player:Focus() > 80 or (Player:BuffP(S.PreciseShotsBuff) or Player:Focus() > 45 and bool(S.Trueshot:CooldownRemainsP()) and Target:TimeToDie() < 25) and Player:BuffDownP(S.TrueshotBuff) or Target:TimeToDie() < 5) then
            return S.ArcaneShot:Cast()
        end
        -- steady_shot
        if S.SteadyShot:IsCastableP() and Player:Focus() < 80  then
